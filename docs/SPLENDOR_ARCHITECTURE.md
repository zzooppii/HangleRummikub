...
## 모바일 보드와 공개 토큰 표시 보정 (2026-09-11)

- 기본판/도시판 모두 시장과 은행의 grid 자식에 최소 너비 0을 적용하고, 800px 이하 단일 열은 `minmax(0, 1fr)`로 제한한다. 4인 기본판 귀족 5장의 intrinsic 너비가 보드 전체를 늘리는 문제를 방지한다. 콘텐츠를 잘라 숨기는 방식은 사용하지 않는다.
- 내 상단의 `▱ n`을 `할인 n`으로 변경한다. 상대 summary에는 보유 토큰 6종(황금 포함)을 항상 표시하며 모바일에서도 숨기지 않는다. 영구 할인과 구매 카드 상세는 펼친 영역에 명시적으로 구분한다.
- 마지막 행동 feedback에 선택적 `tokenMovement: { gained, spent, returned }`를 추가한다. domain이 검증된 행동에서 실제 수령·지불·초과 반환을 별개로 기록한다. 황금이 없는 예약은 획득 0이며 TIMEOUT은 이전 토큰 변화를 이어받지 않는다. 공개 토큰 정보만 포함하고 비공개 예약 카드의 정체는 추가하지 않는다. 기존 feedback의 필드 누락도 읽을 수 있다.

검증: root `npm run typecheck`, `npm test`(shared 124 + web 613 + server 1,661 = 2,398개), `npm run build` 통과. 기존 500 kB 초과 JS chunk 경고는 유지된다. 로컬 브라우저에서 4인 기본판을 진행해 상대 세 명의 토큰 획득과 최근 행동의 색·수량 갱신을 확인했다. 320px·390px·1440px에서 문서 가로 넘침이 없고, 모바일 상대 보유 토큰과 펼친 영구 할인이 구분되어 표시됨을 확인했다.

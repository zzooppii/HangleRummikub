...
CLUE_CORRIDOR_ROWS는 사진의 8열×13줄을 직접 표현하며 방 사각형의 여집합으로 길을 만들지 않는다. CLUE_CELL_PERCENT를 통해 21×21 아트 좌표와 실제 칸 렌더링을 연결한다. 봉투·계단·침실 돌출부의 표시와 통행 마스크는 각각 공개 상수로 관리한다. 회귀 테스트는 통행 69칸, 하단 8/8/8/4칸, 계단 차단, 계단 윗경계 차단, ?칸 좌표 및 벽으로 분리된 위쪽 8칸과 아래쪽 61칸을 검사한다.

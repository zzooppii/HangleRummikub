...
760px 이하에서 진행 중 단계 배너만 `sticky`로 상단에 유지한다. 불투명 배경과 safe-area 여백을 적용하고 긴 단계명은 줄바꿈한다. 대기실·결과 및 데스크톱 배치는 유지한다. 모바일 본문 그리드는 `minmax(0, 1fr)`로 좁은 화면의 가로 넘침을 방지한다.

기존 React 화면의 정적 렌더링과 실제 CSS를 사용하는 로컬 브라우저 레이아웃 검증에서 390px/320px 스크롤 후 배너 top=0, 긴 단계명과 타이머 비겹침, 320px 가로 넘침 없음을 확인했다. 1280px에서는 배너가 기존 static 배치임을 확인했다. 실제 휴대폰 테스트와 실시간 타이머 재검증은 수행하지 않았다. Root typecheck/test/build 및 diff check를 실행하며, 기존 번들 크기 경고는 유지한다.

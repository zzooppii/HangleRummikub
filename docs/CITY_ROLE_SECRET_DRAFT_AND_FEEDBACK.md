...
## 현재 확정 규칙 — 2026-09-10 정정 승인

사용자가 제시하고 적용을 승인한 2인 규칙으로 새 게임을 `city-draft-v3`으로 시작한다. 서버가 1장을 무작위 비공개 제외한 뒤 A는 7장에서 1장만 선택한다. 이후 B는 6장, A는 4장, B는 마지막 2장에서 각각 1장 선택 + 다른 1장 비공개 버리기를 확정한다. 마지막 B도 독립적인 선택 시간과 직접 선택권을 갖는다. 최종 배정 4장(각 2장), 비공개 제외 4장, 공개 제외 0장이다.

첫 A 선택의 `discardRoleId`는 금지하고 이후 세 선택에서는 필수로 검증한다. 시간 초과도 첫 선택은 무작위 한 장, 이후는 서로 다른 두 장을 처리한다. 방장이 정한 10·20·30초(기본 20초)는 각 선택마다 적용한다. 비공개 버리기 목록은 공개하지 않는다. 서버 projection의 `draftDiscardRequired`로 해당 선택의 버리기 필요 여부를 전달한다.

기존 `city-draft-v2` 저장 게임은 아래 과거 방식과 안내를 유지하며, 버전이 없는 저장 게임도 변경하지 않는다. 3–6인 선택은 그대로 유지한다. 9직업 게임이 기권으로 2인이 되면 기존처럼 최초 비공개 제외 2장으로 7장부터 같은 선택 흐름을 시작한다.

## 과거 확정 규칙 변경 — city-draft-v2

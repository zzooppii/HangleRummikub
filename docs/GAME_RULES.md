...
기존 방 선택에서 플레이할 수 있도록 기본판과 온라인 처리 정책을 추가한다. 상세 결정은 [HALLI_GALLI_GAME_RULES.md](./HALLI_GALLI_GAME_RULES.md)를 따른다. 기존 게임의 미확정 항목은 유지한다. 사용자 정정에 따라 할리갈리는 카드가 소진되어 마지막 생존자가 남을 때까지 진행하며, 2인 최종 벨과 15분 강제 종료를 제거한다.

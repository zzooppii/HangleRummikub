...
Roadmap Phase 12까지의 npm workspaces, Room/Session 기반, 브라우저 Lobby, server-authoritative `game:start`와 active Player의 browser-only TurnDraft editor를 구현한 프로젝트다. Submit을 포함한 Game 시작 이후 server mutation은 아직 구현하지 않았다.
...
## Lobby, Game start와 TurnDraft 개발 확인
...
7. 현재 active Player 화면에서 WordGroup과 syllable을 추가하고 rack Tile의 symbol을 골라 초성·중성·종성 slot에 배치한다. 다른 Player 화면의 canonical Board는 바뀌지 않아야 한다.
8. Tile 이동, undo와 reset을 확인한다. active Player tab을 새로고침하면 session은 resume되지만 미제출 TurnDraft는 폐기되어 canonical snapshot에서 다시 시작해야 한다.
...
Phase 12의 rack/Board 편집은 해당 tab 메모리에서만 동작한다. `turn:submit`, draw/pass, timeout countdown과 종료는 후속 Phase 범위다.

...
test("same room: all eleven games can be selected, host starts without any ready commands", async t => {
  for (const gameType of SUPPORTED_GAME_TYPES) {
    await t.test(gameType, async t => {
      const h = await harness(t, (gameType === "JAIPUR" || gameType === "LOST_CITIES") ? 2 : 3), before = await h.sync();
...
test("every game's finished roster returns only remaining members to the same room", async t => {
  for (const gameType of SUPPORTED_GAME_TYPES) await t.test(gameType, async t => {
    const h = await harness(t, (gameType === "JAIPUR" || gameType === "LOST_CITIES") ? 2 : 3);

...
export class RoomPresencePolicyService {
  ...
  async scheduleFinishedRetention(roomId: RoomId): Promise<boolean> {
    ...
    if (
      ...
      room.phase !== "FINISHED" ||
      room.game === null ||
      ((room.gameType === "CITY_ROLE" || room.gameType === "DRAW_RELAY" || room.gameType === "SNEAKY_LUNCH" || room.gameType === "WOLF_NIGHT" || room.gameType === "SPLENDOR" || room.gameType === "HALLI_GALLI" || room.gameType === "ISLAND_SETTLERS") ? room.game.finishedAt === null : room.game.result === null)
    ) {
      return false;
    }
    const finishedAt = (room.gameType === "CITY_ROLE" || room.gameType === "DRAW_RELAY" || room.gameType === "SNEAKY_LUNCH" || room.gameType === "WOLF_NIGHT" || room.gameType === "SPLENDOR" || room.gameType === "HALLI_GALLI" || room.gameType === "ISLAND_SETTLERS") ? room.game.finishedAt : room.game.result?.finishedAt;

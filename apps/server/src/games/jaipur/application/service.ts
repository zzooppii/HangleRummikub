...
import { JaipurClientCommandSchema, GameRevisionSchema, RoomRevisionSchema, ServerTimeSchema, type JaipurClientCommand, type ErrorDto, type RoomId, type PlayerId, type ServerTime } from "@hangul-rummikub/shared";
...
import type { JaipurRoomRecord } from "../../../model/persistence.js";
import { makeJaipurCards, JAIPUR_BONUS_VALUES } from "../domain/catalog.js";
import { createJaipurGame, applyJaipurAction, confirmJaipurRound, type JaipurState, type JaipurRoundSetup } from "../domain/game.js";

export type JaipurDependencies = Readonly<{ roomRepository:RoomRepository; roomUnitOfWork:RoomUnitOfWork; idempotencyRepository:IdempotencyRepository; roomMutationExecutor:RoomMutationSerialExecutor; presence:RoomPresencePolicyReader; clock:Clock; ids:IdGenerator; random:RandomSource }>;
const failure=(code:ErrorDto['code'])=>({ok:false as const,error:{code,message:code==='RULE_VIOLATION'?'선택한 카드와 수량을 확인해주세요.':code==='NOT_ENOUGH_PLAYERS'?'자이푸르는 정확히 2명이 플레이합니다.':'현재 차례와 연결 상태를 확인해주세요.',recoverable:true}});
const Receipt=v.strictObject({outcome:v.literal('ACCEPTED')});
export function transitionJaipur(room:JaipurRoomRecord,state:JaipurState,at:ServerTime):Omit<JaipurRoomRecord,'storageRevision'> {
  if(!room.game)throw new Error('Missing Jaipur game.');
...
export class JaipurService {
  private readonly listeners=new Set<(roomId:RoomId)=>void|Promise<void>>();
  constructor(readonly deps:JaipurDependencies) {}
  ...
  private roundSetup():JaipurRoundSetup {
    const shuffle=<T>(items:T[]):T[]=>{for(let i=items.length-1;i>0;i--){const j=this.deps.random.nextInt(i+1);[items[i],items[j]]=[items[j]!,items[i]!];}return items;};
    // Reserve the three market camels before shuffling, so the remaining deck is uniform.
    const cards=makeJaipurCards(()=>this.deps.ids.generateTileId());
    const marketCamels=cards.filter(c=>c.type==='CAMEL').slice(0,3);
    const marketIds=new Set(marketCamels.map(c=>c.cardId));
    return {cards:[...marketCamels,...shuffle(cards.filter(c=>!marketIds.has(c.cardId)))],bonusBank:JAIPUR_BONUS_VALUES.map(b=>({size:b.size,values:shuffle([...b.values])}))};
  }
  async start(input:StartGameInput):Promise<GameStartResult> {
    ...
    try {return await d.roomMutationExecutor.run(input.roomId,async ():Promise<GameStartResult>=>{
      ...
      if(room?.gameType!=='JAIPUR'||room.departedPlayerIds?.includes(input.actorPlayerId)||!room.players.some(p=>p.playerId===input.actorPlayerId))return failure('INVALID_PHASE');
      ...
      const state=createJaipurGame({...this.roundSetup(),gameId,playerIds:room.players.map(p=>p.playerId),now,transitionId:turnId,starter:d.random.nextInt(2)});
  ...
  async command(input:Readonly<{roomId:RoomId;actorPlayerId:PlayerId;command:JaipurClientCommand;receivedAt:ServerTime;authorization:{isCurrent():boolean}}>) {
    const parsed=v.safeParse(JaipurClientCommandSchema,input.command);if(!parsed.success)return failure('INVALID_PAYLOAD');
    ...
    try {
      const result=await d.roomMutationExecutor.run(input.roomId,async()=>{
        ...
        if(c.kind==='jaipur:act' && c.turnId!==s.transitionId || c.kind==='jaipur:nextRound' && c.roundId!==s.roundId)return failure('STALE_GAME_REVISION');
        if(c.kind==='jaipur:nextRound'&&(s.phase!=='ROUND_RESULT'||s.confirmedPlayerIds.includes(input.actorPlayerId)))return failure('INVALID_PHASE');
        const applied=c.kind==='jaipur:act'?applyJaipurAction(s,input.actorPlayerId,c.payload,now,d.ids.generateTurnId()):confirmJaipurRound(s,input.actorPlayerId,s.confirmedPlayerIds.length===1?this.roundSetup():null,d.ids.generateTurnId());
        if(!applied.ok)return failure(applied.reason==='INVALID_ACTION'?'RULE_VIOLATION':applied.reason);
        const committed=await d.roomUnitOfWork.commit({roomMutation:{kind:'REPLACE',candidate:transitionJaipur(room,applied.state,now),expectedRoomRevision:room.roomRevision,expectedStorageRevision:room.storageRevision},sessionMutation:{kind:'NONE'},idempotency:{scopeKey,requestId:c.requestId,payloadFingerprint,terminalResult:{outcome:'ACCEPTED'},createdAt:now}},{isSatisfied:()=>input.authorization.isCurrent()});

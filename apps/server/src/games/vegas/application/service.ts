...
import { RequestIdSchema, VegasClientCommandSchema, GameRevisionSchema, RoomRevisionSchema, ServerTimeSchema, type VegasClientCommand, type ErrorDto, type RoomId, type PlayerId, type ServerTime } from "@hangul-rummikub/shared";
...
import type { VegasRoomRecord } from "../../../model/persistence.js";
import { timeoutVegas, createVegasGame, applyVegasAction, type VegasState } from "../domain/game.js";
export type VegasDependencies = Readonly<{
...
const failure = (code: ErrorDto['code']) => ({ ok: false as const, error: { code, message: code === 'RULE_VIOLATION' ? '주사위를 굴린 뒤 나온 숫자 하나를 선택해주세요.' : code === 'NOT_ENOUGH_PLAYERS' ? '라스베이거스는 2~5명이 플레이합니다.' : '현재 차례와 연결 상태를 확인해주세요.', recoverable: true } });
const Receipt = v.strictObject({ outcome: v.literal('ACCEPTED') });
export function transitionVegas(room: VegasRoomRecord, state: VegasState, at: ServerTime): Omit<VegasRoomRecord, 'storageRevision'> {
    if (!room.game)
        throw new Error('Missing Vegas game.');
...
export class VegasService {
    ...
    constructor(readonly deps: VegasDependencies) { }
    ...
    private async cancelTimer(turnId: ScheduledTurnDeadline['turnId']) {
        try {
            ...
            console.error('Vegas timer cancellation failed; stale callbacks are guarded.');
        }
    }
    async schedule(roomId: RoomId) {
        try {
            const room = await this.deps.roomRepository.findById(roomId);
            if (room?.gameType !== 'VEGAS' || room.phase !== 'PLAYING' || !room.game || room.game.state.deadlineAt === null)
                return;
            ...
            console.error('Vegas scheduling failed; overdue recovery will retry.');
        }
    }
    async timeout(input: ScheduledTurnDeadline): Promise<{
        ...
        try {
            const applied = await d.roomMutationExecutor.run(input.roomId, async () => {
                ...
                if (room?.gameType !== 'VEGAS' || room.phase !== 'PLAYING' || !room.game || room.game.gameId !== input.gameId || room.game.gameRevision !== input.expectedGameRevision || room.game.state.transitionId !== input.turnId || room.game.state.deadlineAt !== input.deadlineAt || now < input.deadlineAt)
                    return false;
                const state = timeoutVegas(room.game.state, now, d.ids.generateTurnId(), d.random);
                if (!state)
                    return false;
                const committed = await d.roomUnitOfWork.commit({ roomMutation: { kind: 'REPLACE', candidate: transitionVegas(room, state, now), expectedRoomRevision: room.roomRevision, expectedStorageRevision: room.storageRevision }, sessionMutation: { kind: 'NONE' }, idempotency: { scopeKey: `room-timeout:${room.roomId}:${room.game.gameId}`, requestId: v.parse(RequestIdSchema, `turn:${input.turnId}`), payloadFingerprint: JSON.stringify([input.turnId, input.deadlineAt]), terminalResult: { transitioned: true }, createdAt: now } });
    ...
    async start(input: StartGameInput): Promise<GameStartResult> {
        ...
        try {
            const result = await d.roomMutationExecutor.run(input.roomId, async (): Promise<GameStartResult> => {
                ...
                if (room?.gameType !== 'VEGAS' || room.departedPlayerIds?.includes(input.actorPlayerId) || !room.players.some(p => p.playerId === input.actorPlayerId))
                ...
                const state = createVegasGame({ gameId, playerIds: room.players.map(p => p.playerId), now, turnId, starter: d.random.nextInt(room.players.length), random: d.random });
    ...
    async command(input: Readonly<{
        ...
        command: VegasClientCommand;
        receivedAt: ServerTime;
        authorization: {
            isCurrent(): boolean;
        };
    }>) {
        const parsed = v.safeParse(VegasClientCommandSchema, input.command);
        if (!parsed.success)
        ...
        try {
            const result = await d.roomMutationExecutor.run(input.roomId, async () => {
                ...
                if (c.turnId !== s.transitionId)
                    return failure('STALE_GAME_REVISION');
                const applied = applyVegasAction(s, input.actorPlayerId, c.payload, now, d.ids.generateTurnId(), d.random);
                if (!applied.ok)
                    return failure(applied.reason === 'INVALID_ACTION' ? 'RULE_VIOLATION' : applied.reason);
                const committed = await d.roomUnitOfWork.commit({ roomMutation: { kind: 'REPLACE', candidate: transitionVegas(room, applied.state, now), expectedRoomRevision: room.roomRevision, expectedStorageRevision: room.storageRevision }, sessionMutation: { kind: 'NONE' }, idempotency: { scopeKey, requestId: c.requestId, payloadFingerprint, terminalResult: { outcome: 'ACCEPTED' }, createdAt: now } }, { isSatisfied: () => input.authorization.isCurrent() });

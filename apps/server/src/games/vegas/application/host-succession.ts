...
import type { VegasDependencies } from "./service.js";
/** Process-local presence history, like the connection registry. Never owns game cards or player actions. */
export class VegasHostSuccession {
    readonly offline = new Map<RoomId, Map<PlayerId, ServerTime>>();
    private timer: ReturnType<typeof setInterval> | undefined;
    constructor(private readonly deps: VegasDependencies, private readonly notify: (roomId: RoomId) => Promise<void>) { }
    ...
    resumed(roomId: RoomId, playerId: PlayerId): void {
        ...
        void this.evaluate(roomId).catch(() => console.error("VEGAS Host policy reconciliation failed."));
    }
    start(): void {
        if (this.timer)
            return;
        this.timer = setInterval(() => {
            for (const roomId of this.offline.keys())
                void this.evaluate(roomId).catch(() => console.error("VEGAS Host policy retry failed."));
        }, 1000);
        this.timer.unref();
    }
    stop(): void {

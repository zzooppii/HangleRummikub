...
export function projectCityRoleV2Game(input: {
  ...
  const common = {
    ...(e === undefined ? {} : { expansion: { settings: { enabled: e.settings.enabled, roles: [...e.settings.roles], ...(e.settings.selectionSeconds === undefined ? {} : { selectionSeconds: e.settings.selectionSeconds }) }, specialIds: [...e.specialIds], tax: e.tax, decorated: [...e.decorated], museum: e.museum.map(row => ({ buildingId: row.buildingId, count: row.cards.length })), disabledRole: state.marks.find(m => m.kind === "DISABLE" && m.status !== "CANCELLED")?.targetRoleId ?? null, robbedRole: state.marks.find(m => m.kind === "GOLD_TRANSFER" && m.status !== "CANCELLED")?.targetRoleId ?? null, warrants: [...(e.warrants?.roles ?? [])].sort(), threats: [...(e.threats?.roles ?? [])].sort(), witchTarget: e.witch?.targetRoleId ?? null, pending: e.pending?.kind ?? null,

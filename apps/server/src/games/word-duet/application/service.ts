...
import { DuetClientCommandSchema, GameRevisionSchema, RoomRevisionSchema, ServerTimeSchema, type DuetClientCommand, type ErrorDto, type RoomId, type PlayerId, type ServerTime } from "@hangul-rummikub/shared";
...
import type { DuetRoomRecord } from "../../../model/persistence.js";
import { DuetCardIdSchema } from "@hangul-rummikub/shared";
import { DUET_WORDS } from "../domain/words.js";
import { createDuetGame, applyDuetAction, DUET_ROLE_PAIRS, type DuetState } from "../domain/game.js";

export type DuetDependencies = Readonly<{ roomRepository:RoomRepository; roomUnitOfWork:RoomUnitOfWork; idempotencyRepository:IdempotencyRepository; roomMutationExecutor:RoomMutationSerialExecutor; presence:RoomPresencePolicyReader; clock:Clock; ids:IdGenerator; random:RandomSource }>;
const failure=(code:ErrorDto['code'])=>({ok:false as const,error:{code,message:code==='RULE_VIOLATION'?'현재 단계와 선택한 단어, 힌트를 확인해주세요.':code==='NOT_ENOUGH_PLAYERS'?'코드네임 듀엣은 정확히 2명이 플레이합니다.':'현재 차례와 연결 상태를 확인해주세요.',recoverable:true}});
const Receipt=v.strictObject({outcome:v.literal('ACCEPTED')});
export function transitionDuet(room:DuetRoomRecord,state:DuetState,at:ServerTime):Omit<DuetRoomRecord,'storageRevision'> {
  if(!room.game)throw new Error('Missing Duet game.');
...
export class DuetService {
  ...
  constructor(readonly deps:DuetDependencies) {}
  ...
  private setup():DuetState['cards'] {
    ...
    const words=shuffle([...DUET_WORDS]).slice(0,25),pairs=shuffle([...DUET_ROLE_PAIRS]);
    return words.map((word,i)=>({cardId:v.parse(DuetCardIdSchema,this.deps.ids.generateTileId()),word,roles:[pairs[i]![0],pairs[i]![1]],foundBy:null,bystanderFor:[]}));
  }
  async start(input:StartGameInput):Promise<GameStartResult> {
    ...
    try {return await d.roomMutationExecutor.run(input.roomId,async ():Promise<GameStartResult>=>{
      ...
      if(room?.gameType!=='WORD_DUET'||room.departedPlayerIds?.includes(input.actorPlayerId)||!room.players.some(p=>p.playerId===input.actorPlayerId))return failure('INVALID_PHASE');
      ...
      const state=createDuetGame({cards:this.setup(),gameId,playerIds:room.players.map(p=>p.playerId),now,transitionId:turnId});
  ...
  async command(input:Readonly<{roomId:RoomId;actorPlayerId:PlayerId;command:DuetClientCommand;receivedAt:ServerTime;authorization:{isCurrent():boolean}}>) {
    const parsed=v.safeParse(DuetClientCommandSchema,input.command);if(!parsed.success)return failure('INVALID_PAYLOAD');

...
const StateSchema = v.strictObject({ gameId: GameIdSchema, rulesVersion: v.literal("halli-galli-v2"), revision: GameRevisionSchema,
...
export function parseHalliState(input: unknown): HalliState {
 ...
 if (s.phase === "PLAYING") {
  if (s.result !== null || s.finishedAt !== null || s.nextTransitionAt === null || !s.players.some(p => p.playerId === s.activePlayerId && !p.eliminated) || s.players.filter(p => !p.eliminated).length < 2) throw new Error("HALLI running state failure.");
 ...
 if (s.result?.reason === "LAST_PLAYER" && (s.players.filter(p => !p.eliminated).length !== 1 || s.players.some(p => p.discard.length > 0) || s.players.find(p => !p.eliminated)?.deck.length !== 56)) throw new Error("HALLI unfinished exhaustion result.");
 return s;
}
function resultFor(s: HalliState, reason: NonNullable<HalliState["result"]>["reason"]): NonNullable<HalliState["result"]> {
 ...
 return { reason, scores, winnerPlayerIds: reason === "CANCELLED" ? [] : s.players.filter(p => !p.eliminated).map(p => p.playerId) };
}
...
function settle(s: HalliState, now: number): void {
 ...
 if (survivors.length === 1) {
  ...
  collect(s, winner.playerId); s.activePlayerId = winner.playerId;
  finish(s, now, "LAST_PLAYER");
 }
}
...
export function flipHalli(state: HalliState, actor: PlayerId, now: number, token: string, automatic = false): HalliState | null {
 if (state.phase !== "PLAYING" || state.activePlayerId !== actor || (!automatic && (now < state.flipAvailableAt || now >= state.nextTransitionAt!))) return null;

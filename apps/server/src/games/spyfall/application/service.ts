import { GameRevisionSchema, RoomRevisionSchema, ServerTimeSchema, TurnIdSchema, RequestIdSchema,
  SpyfallClientCommandSchema, SPYFALL_DEFAULT_SETTINGS, type SpyfallClientCommand, type ErrorDto, type RoomId, type PlayerId, type ServerTime } from "@hangul-rummikub/shared";
...
import type { SpyfallRoomRecord } from "../../../model/persistence.js";
import { createSpyfallGame, commandSpyfall, advanceSpyfall, type SpyfallState } from "../domain/game.js";

import { chooseSpyfallLocation } from "../domain/locations.js";

export type SpyfallDependencies = Readonly<{ roomRepository: RoomRepository; roomUnitOfWork: RoomUnitOfWork; idempotencyRepository: IdempotencyRepository;
...
export function transitionSpyfall(room: SpyfallRoomRecord, state: SpyfallState, at: ServerTime): Omit<SpyfallRoomRecord, "storageRevision"> {
  if (!room.game) throw new Error("Missing SPYFALL game.");
...
export class SpyfallService {
  ...
  constructor(readonly deps: SpyfallDependencies) {}
  ...
  async schedule(roomId: RoomId) {
    ...
    if (room?.gameType !== "SPYFALL" || !room.game || room.phase !== "PLAYING" || room.game.state.nextTransitionAt === null) return;
    ...
    } catch { console.error("SPYFALL scheduling failed; overdue recovery will retry."); }
  }
  private async cancelTimer(turnId: string) {
    try { await this.deps.turnScheduler.cancelTimeout(v.parse(TurnIdSchema, turnId)); }
    catch { console.error("SPYFALL timer cancellation failed; stale callbacks are guarded."); }
  }
  async start(input: StartGameInput): Promise<GameStartResult> {
    ...
    try {
      const result = await d.roomMutationExecutor.run(input.roomId, async (): Promise<GameStartResult> => {
        ...
        if (room?.gameType !== "SPYFALL") return failure("INVALID_PHASE");
        ...
        const settings = room.settings ?? SPYFALL_DEFAULT_SETTINGS;
        const playerIds = room.players.map(p => p.playerId);
        const spyPlayerId = playerIds[d.random.nextInt(playerIds.length)]!;
        for (let i = playerIds.length - 1; i > 0; i--) { const j = d.random.nextInt(i + 1); [playerIds[i], playerIds[j]] = [playerIds[j]!, playerIds[i]!]; }
        const state = createSpyfallGame({ gameId, playerIds, spyPlayerId, settings,
          location: chooseSpyfallLocation(settings.locationPack, d.random), now, transitionId: turnId });
  ...
  async command(input: Readonly<{ roomId: RoomId; actorPlayerId: PlayerId; command: SpyfallClientCommand; receivedAt: ServerTime; authorization: { isCurrent(): boolean } }>) {
    const parsed = v.safeParse(SpyfallClientCommandSchema, input.command);
    ...
    try {
      const result = await d.roomMutationExecutor.run(input.roomId, async () => {
        if (!input.authorization.isCurrent()) return failure("UNAUTHENTICATED");
        const room = await d.roomRepository.findById(input.roomId);
        if (room?.gameType !== "SPYFALL" || room.departedPlayerIds?.includes(input.actorPlayerId) || !room.players.some(p => p.playerId === input.actorPlayerId)) return failure("INVALID_PHASE");
        ...
        const now = d.clock.now(); let candidate: Omit<SpyfallRoomRecord, "storageRevision"> = room;
        let outcome: v.InferOutput<typeof Receipt>["outcome"];
        if (c.kind === "spyfall:configure") {
          ...
          const state = commandSpyfall(room.game.state, input.actorPlayerId, c, now, d.ids.generateTurnId());
          if (!state) return failure("INVALID_PAYLOAD");
          outcome = "ACCEPTED"; candidate = transitionSpyfall(room, state, now);
        }
  ...
  async timeout(input: ScheduledTurnDeadline): Promise<{ status: "NO_OP" | "APPLIED" | "FAILED" }> {
    ...
    try {
      const applied = await d.roomMutationExecutor.run(input.roomId, async () => {
        ...
        if (room?.gameType !== "SPYFALL" || room.phase !== "PLAYING" || !room.game || room.game.gameId !== input.gameId ||
        ...
        const state = advanceSpyfall(previous, now, d.ids.generateTurnId());
        if (!state) return false;
        const committed = await d.roomUnitOfWork.commit({ roomMutation: { kind: "REPLACE", candidate: transitionSpyfall(room, state, now),
          expectedRoomRevision: room.roomRevision, expectedStorageRevision: room.storageRevision }, sessionMutation: { kind: "NONE" },
          idempotency: { scopeKey: `spyfall-timer:${room.roomId}:${room.game.gameId}`, requestId: v.parse(RequestIdSchema, `phase:${input.turnId}`),

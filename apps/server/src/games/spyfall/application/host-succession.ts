...
import type { SpyfallDependencies } from "./service.js";

/** Process-local presence history, like the connection registry. Never owns secret words, roles or ballots. */
export class SpyfallHostSuccession {
  readonly offline = new Map<RoomId, Map<PlayerId, ServerTime>>();
  private timer: ReturnType<typeof setInterval> | undefined;
  constructor(private readonly deps: SpyfallDependencies, private readonly notify: (roomId: RoomId) => Promise<void>) {}
  ...
  resumed(roomId: RoomId, playerId: PlayerId): void {
    ...
    void this.evaluate(roomId).catch(() => console.error("SPYFALL Host policy reconciliation failed."));
  }
  start(): void {
    if (this.timer) return;
    this.timer = setInterval(() => {
      for (const roomId of this.offline.keys()) void this.evaluate(roomId).catch(() => console.error("SPYFALL Host policy retry failed."));
    }, 1000);
    this.timer.unref();
  }
  ...
  async evaluate(roomId: RoomId): Promise<boolean> {
    ...
    const transferred = await d.roomMutationExecutor.run(roomId, async () => {
      ...
      if (!room || room.gameType !== "SPYFALL") { this.offline.delete(roomId); return false; }
      ...
      const result = await d.roomUnitOfWork.commit({ roomMutation: { kind: "REPLACE",
        ...
        idempotency: { scopeKey: `spyfall-host:${roomId}`, requestId: parse(RequestIdSchema, `host:${room.roomRevision}:${since}`),

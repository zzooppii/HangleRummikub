...
import { ClueClientCommandSchema, GameRevisionSchema, RoomRevisionSchema, ServerTimeSchema, type ClueClientCommand, type ErrorDto, type RoomId, type PlayerId, type ServerTime } from "@hangul-rummikub/shared";
...
import type { ClueRoomRecord } from "../../../model/persistence.js";
import { createClueGame, applyClueAction, makeClueCards, type ClueState } from "../domain/game.js";

export type ClueDependencies = Readonly<{ roomRepository:RoomRepository; roomUnitOfWork:RoomUnitOfWork; idempotencyRepository:IdempotencyRepository; roomMutationExecutor:RoomMutationSerialExecutor; presence:RoomPresencePolicyReader; clock:Clock; ids:IdGenerator; random:RandomSource }>;
const failure=(code:ErrorDto['code'])=>({ok:false as const,error:{code,message:code==='RULE_VIOLATION'?'현재 단계와 이동, 추리 또는 제시할 카드를 확인해주세요.':code==='NOT_ENOUGH_PLAYERS'?'클루는 3~6명이 플레이합니다.':'현재 차례와 연결 상태를 확인해주세요.',recoverable:true}});
const Receipt=v.strictObject({outcome:v.literal('ACCEPTED')});
export function transitionClue(room:ClueRoomRecord,state:ClueState,at:ServerTime):Omit<ClueRoomRecord,'storageRevision'> {
  if(!room.game)throw new Error('Missing Clue game.');
...
export class ClueService {
  ...
  constructor(readonly deps:ClueDependencies) {}
  ...
  async start(input:StartGameInput):Promise<GameStartResult> {
    ...
    try {return await d.roomMutationExecutor.run(input.roomId,async ():Promise<GameStartResult>=>{
      ...
      if(room?.gameType!=='CLUE'||room.departedPlayerIds?.includes(input.actorPlayerId)||!room.players.some(p=>p.playerId===input.actorPlayerId))return failure('INVALID_PHASE');
      ...
      if((room.players.length<3||room.players.length>6))return failure('NOT_ENOUGH_PLAYERS');
      ...
      const state=createClueGame({cards:makeClueCards(()=>d.ids.generateTileId()),gameId,playerIds:room.players.map(p=>p.playerId),now,turnId,random:d.random});
  ...
  async command(input:Readonly<{roomId:RoomId;actorPlayerId:PlayerId;command:ClueClientCommand;receivedAt:ServerTime;authorization:{isCurrent():boolean}}>) {
    const parsed=v.safeParse(ClueClientCommandSchema,input.command);if(!parsed.success)return failure('INVALID_PAYLOAD');
    ...
    try {
      const result=await d.roomMutationExecutor.run(input.roomId,async()=>{
        ...
        const applied=applyClueAction(s,input.actorPlayerId,c.payload,now,d.ids.generateTurnId(),d.random);
        if(!applied.ok)return failure(applied.reason==='INVALID_ACTION'?'RULE_VIOLATION':applied.reason);
        const committed=await d.roomUnitOfWork.commit({roomMutation:{kind:'REPLACE',candidate:transitionClue(room,applied.state,now),expectedRoomRevision:room.roomRevision,expectedStorageRevision:room.storageRevision},sessionMutation:{kind:'NONE'},idempotency:{scopeKey,requestId:c.requestId,payloadFingerprint,terminalResult:{outcome:'ACCEPTED'},createdAt:now}},{isSatisfied:()=>input.authorization.isCurrent()});

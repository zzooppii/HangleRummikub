...
export class ClueHostSuccession {
  ...
  resumed(roomId: RoomId, playerId: PlayerId): void {
    ...
    void this.evaluate(roomId).catch(() =>
      console.error("CLUE Host policy reconciliation failed."),
    );
  }
  start(): void {
    ...
    this.timer = setInterval(() => {
      for (const roomId of this.offline.keys())
        void this.evaluate(roomId).catch(() =>
          console.error("CLUE Host policy retry failed."),
        );
    }, 1000);
    this.timer.unref();
  }

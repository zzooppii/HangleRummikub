...
import type { BurgundyRoomRecord } from "../../../model/persistence.js";
import {
  timeoutBurgundy,
  createBurgundyGame,
  applyBurgundyAction,
...
export type BurgundyDependencies = Readonly<{
...
const failure = (code: ErrorDto["code"], reason?: string) => ({
  ok: false as const,
  error: {
    code,
    message:
      reason ??
      (code === "RULE_VIOLATION"
        ? "주사위·타일·배치 위치와 추가 효과 선택을 확인해주세요."
        : code === "NOT_ENOUGH_PLAYERS"
          ? "버건디의 성은 2~4명이 플레이합니다."
          : "현재 차례와 연결 상태를 확인해주세요."),
    recoverable: true,
  },
});
const Receipt = v.strictObject({ outcome: v.literal("ACCEPTED") });
export function transitionBurgundy(
  room: BurgundyRoomRecord,
  state: BurgundyState,
  at: ServerTime,
): Omit<BurgundyRoomRecord, "storageRevision"> {
  if (!room.game) throw new Error("Missing Burgundy game.");
...
export class BurgundyService {
  private readonly listeners = new Set<
    (roomId: RoomId) => void | Promise<void>
  >();
  constructor(readonly deps: BurgundyDependencies) {}
  ...
  private async cancelTimer(turnId: ScheduledTurnDeadline["turnId"]) {
    try {
      ...
      console.error(
        "Burgundy timer cancellation failed; stale callbacks are guarded.",
      );
    }
  }
  async schedule(roomId: RoomId) {
    try {
      ...
      console.error("Burgundy scheduling failed; overdue recovery will retry.");
    }
  }
  async timeout(
      ...
      const applied = await d.roomMutationExecutor.run(
        ...
        async () => {
          ...
          const committed = await d.roomUnitOfWork.commit({
            roomMutation: {
              kind: "REPLACE",
              candidate: transitionBurgundy(room, state, now),
    ...
    try {
      const result = await d.roomMutationExecutor.run(
        ...
        async (): Promise<GameStartResult> => {
          ...
          const settings = room.settings ?? BURGUNDY_DEFAULT_SETTINGS;
          const state = createBurgundyGame(
            {
              tiles: makeBurgundyTiles(() => d.ids.generateTileId(), settings),
              gameId,
              playerIds: room.players.map((p) => p.playerId),
              now,
              generateTileId: () => d.ids.generateTileId(),
              transitionId: turnId,
  ...
  async command(
    input: Readonly<{
      ...
      command: BurgundyClientCommand;
      receivedAt: ServerTime;
      authorization: { isCurrent(): boolean };
    }>,
  ) {
    const parsed = v.safeParse(BurgundyClientCommandSchema, input.command);
    ...
    try {
      const result = await d.roomMutationExecutor.run(
        ...
        async () => {
          ...
          if (c.turnId !== s.transitionId)
            return failure("STALE_GAME_REVISION");
          const applied = applyBurgundyAction(
            s,
            input.actorPlayerId,
            c.payload,
            now,
          ...
          if (!applied.ok) return failure(applied.code, applied.reason);
          const committed = await d.roomUnitOfWork.commit(
            {
              roomMutation: {
                kind: "REPLACE",
                candidate: transitionBurgundy(room, applied.state, now),
                expectedRoomRevision: room.roomRevision,
                expectedStorageRevision: room.storageRevision,
              },
              sessionMutation: { kind: "NONE" },
      ...
      if (changed) {
        if (c.kind === "burgundy:act") await this.cancelTimer(c.turnId);
        await this.schedule(input.roomId);
        await this.notify(input.roomId);
      }
      return result;

...
import type { SplendorRoomRecord } from "../../../model/persistence.js";
import {
  createSplendorGame,
  applySplendorAction,
  timeoutSplendor,
...
export type SplendorDependencies = Readonly<{
...
const failure = (code: ErrorDto["code"]) => ({
  ...
  error: {
    code,
    message: "현재 단계와 연결 상태를 확인해주세요.",
    recoverable: true,
  },
});
const Receipt = v.strictObject({
  outcome: v.picklist(["ACCEPTED", "CONFIGURED", "REMATCHED"]),
});
const receiptResult = (value: unknown) => {
  const receipt = v.parse(Receipt, value);
  return { ok: true as const, outcome: receipt.outcome };
};
export function transitionSplendor(
  room: SplendorRoomRecord,
  state: SplendorState,
  at: ServerTime,
): Omit<SplendorRoomRecord, "storageRevision"> {
  if (!room.game) throw new Error("Missing SPLENDOR game.");
  ...
  return {
    ...
    game: {
      ...room.game,
      state,
      gameRevision: v.parse(GameRevisionSchema, state.revision),
      finishedAt:
        state.finishedAt === null
          ? null
          : v.parse(ServerTimeSchema, state.finishedAt),
    },
    roomRevision:
      phase === room.phase
        ? room.roomRevision
...
export class SplendorService {
  private readonly listeners = new Set<
    (roomId: RoomId) => void | Promise<void>
  >();
  constructor(readonly deps: SplendorDependencies) {}
  ...
  async schedule(roomId: RoomId) {
    try {
      ...
      await this.deps.turnScheduler.scheduleTimeout({
        ...
        turnId: v.parse(TurnIdSchema, room.game.state.transitionId),
        deadlineAt: v.parse(ServerTimeSchema, room.game.state.nextTransitionAt),
      });
    } catch {
      console.error("SPLENDOR scheduling failed; overdue recovery will retry.");
    }
  }
  private async cancelTimer(turnId: string) {
    try {
      await this.deps.turnScheduler.cancelTimeout(
        v.parse(TurnIdSchema, turnId),
      );
    } catch {
      console.error(
        "SPLENDOR timer cancellation failed; stale callbacks are guarded.",
      );
    }
  }
    ...
    try {
      const result = await d.roomMutationExecutor.run(
        ...
        async (): Promise<GameStartResult> => {
          ...
          const cards = makeSplendorCards(() => d.ids.generateTileId());
          const nobles = [...SPLENDOR_NOBLES],
            playerIds = [...room.players]
              .sort((a, b) => a.joinOrder - b.joinOrder)
              .map((p) => p.playerId);
          function shuffle<T>(items: T[]) {
            for (let i = items.length - 1; i > 0; i--) {
              const j = d.random.nextInt(i + 1);
  ...
  async command(
    input: Readonly<{
      ...
      command: SplendorClientCommand;
      receivedAt: ServerTime;
      authorization: { isCurrent(): boolean };
    }>,
  ) {
    const parsed = v.safeParse(SplendorClientCommandSchema, input.command);
    ...
    try {
      const result = await d.roomMutationExecutor.run(
        ...
        async () => {
          ...
          if (prior.status === "CONFLICT") return failure("REQUEST_ID_REUSED");
          if (prior.status === "REPLAY")
            return receiptResult(prior.record.terminalResult);
          const now = d.clock.now();
  ...
  ): Promise<{ status: "NO_OP" | "APPLIED" | "FAILED" }> {
    const d = this.deps;
    try {
      const applied = await d.roomMutationExecutor.run(
        ...
        async () => {
          ...
          if (
            ...
            room.game.state.nextTransitionAt !== input.deadlineAt ||
            now < input.deadlineAt
          )
            return false;
          const previous = room.game.state;
          const state = timeoutSplendor(previous, now, d.ids.generateTurnId());
          if (!state) return false;
          const committed = await d.roomUnitOfWork.commit({
            roomMutation: {
              kind: "REPLACE",
              candidate: transitionSplendor(room, state, now),
              expectedRoomRevision: room.roomRevision,
              expectedStorageRevision: room.storageRevision,
            },
            sessionMutation: { kind: "NONE" },
            idempotency: {
              ...
              requestId: v.parse(RequestIdSchema, `phase:${input.turnId}`),
              payloadFingerprint: JSON.stringify([
                input.turnId,
                input.deadlineAt,
              ]),

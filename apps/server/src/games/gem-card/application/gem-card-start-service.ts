import { RoomRevisionSchema, type ErrorDto, } from "@hangul-rummikub/shared";
import { parse, safeParse } from "valibot";
import type { GameStartResult, GameStartSuccessData, StartGameInput, } from "../../../application/game-start-service.js";
...
import type { RoomPresenceLease, RoomPresencePolicyReader, } from "../../../ports/room-presence-policy.js";
import type { RoomUnitOfWork, RoomUnitOfWorkResult, } from "../../../ports/room-unit-of-work.js";
...
import { createInitialGemGameState } from "../domain/game-state.js";
import { ServerTimeSchema } from "@hangul-rummikub/shared";
import { GEM_CARDSET_V1 } from "../domain/cardset-v1.js";
import { createGemTurn, GEM_TURN_DURATION_MS } from "../domain/turn.js";
import { shuffleFrozen } from "../../../domain/frozen-fisher-yates.js";
import { GEM_CARD_GAME_TYPE } from "../gem-card-registration.js";
export type GemCardStartServiceDependencies = Readonly<{
...
function parseReplay(terminalResult: IdempotencyRecord["terminalResult"]): GameStartResult {
  const parsed = safeParse(GameStartSuccessDataSchema, terminalResult);
  return parsed.success
    ? succeeded(parsed.output)
    : failed(ERRORS.INTERNAL_ERROR);
}
export class GemCardStartService {
  readonly #dependencies: GemCardStartServiceDependencies;
  constructor(dependencies: GemCardStartServiceDependencies) {
    this.#dependencies = dependencies;
  }
  async start(input: StartGameInput): Promise<GameStartResult> {
    let result: GameStartResult;
    try {
      result = await this.#dependencies.roomMutationExecutor.run(input.roomId, () => this.#startWithinRoomBoundary(input));
    }
    catch {
      return failed(ERRORS.INTERNAL_ERROR);
    }
    if (result.ok) {
      await scheduleCurrentTurnBestEffort(this.#dependencies.roomRepository, this.#dependencies.turnScheduler, {
  ...
  async #startWithinRoomBoundary(input: StartGameInput): Promise<GameStartResult> {
    const scopeKey = scope(input);
    const payloadFingerprint = fingerprint(input);
    const prior = await this.#dependencies.idempotencyRepository.classify(scopeKey, input.requestId, payloadFingerprint);
    ...
    if (room.gameType !== GEM_CARD_GAME_TYPE) {
      return failed(ERRORS.INTERNAL_ERROR);
    }
    ...
    const presenceLease = await this.#dependencies.presenceLeaseReader.acquireRoomPresenceLease(room.roomId);
    if (!room.players.every((player) => presenceLease.connectionStatusByPlayerId.get(player.playerId) ===
      "CONNECTED")) {
    ...
    const turnOrder = shuffleFrozen(playerIds, this.#dependencies.randomSource);
    const activePlayerId = turnOrder[0];
    if (activePlayerId === undefined)
      throw new Error("GEM initial player missing.");
    const deck = (tier: 1 | 2 | 3) => shuffleFrozen(GEM_CARDSET_V1.filter(card => card.tier === tier).map(card => card.cardId), this.#dependencies.randomSource);
    const game = createInitialGemGameState({
      gameId: this.#dependencies.idGenerator.generateGameId(), playerIds, turnOrder,
      tierDecks: { 1: deck(1), 2: deck(2), 3: deck(3) },
      initialTurn: createGemTurn({ turnId: this.#dependencies.idGenerator.generateTurnId(), turnNumber: 1,
        activePlayerId, startedAt, deadlineAt: parse(ServerTimeSchema, startedAt + GEM_TURN_DURATION_MS) }),
    });
  ...
  #mapCommit(result: RoomUnitOfWorkResult, input: StartGameInput, presenceLease: RoomPresenceLease): GameStartResult {

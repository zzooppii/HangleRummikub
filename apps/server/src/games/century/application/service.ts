...
import { CenturyClientCommandSchema, GameRevisionSchema, RoomRevisionSchema, ServerTimeSchema, type CenturyClientCommand, type ErrorDto, type RoomId, type PlayerId, type ServerTime } from "@hangul-rummikub/shared";
...
import type { CenturyRoomRecord } from "../../../model/persistence.js";
import { makeCenturyCards } from "../domain/catalog.js";
import { createCenturyGame, applyCenturyAction, type CenturyState } from "../domain/game.js";

export type CenturyDependencies = Readonly<{ roomRepository:RoomRepository; roomUnitOfWork:RoomUnitOfWork; idempotencyRepository:IdempotencyRepository; roomMutationExecutor:RoomMutationSerialExecutor; presence:RoomPresencePolicyReader; clock:Clock; ids:IdGenerator; random:RandomSource }>;
const failure=(code:ErrorDto['code'])=>({ok:false as const,error:{code,message:code==='RULE_VIOLATION'?'선택한 카드와 수량을 확인해주세요.':code==='NOT_ENOUGH_PLAYERS'?'센추리는 2–5명이 플레이합니다.':'현재 차례와 연결 상태를 확인해주세요.',recoverable:true}});
const Receipt=v.strictObject({outcome:v.literal('ACCEPTED')});
export function transitionCentury(room:CenturyRoomRecord,state:CenturyState,at:ServerTime):Omit<CenturyRoomRecord,'storageRevision'> {
  if(!room.game)throw new Error('Missing Century game.');
...
export class CenturyService {
  ...
  constructor(readonly deps:CenturyDependencies) {}
  ...
  private setup(players: number) {
    ...
    const cards=makeCenturyCards(players,()=>this.deps.ids.generateTileId());
    return {merchants:[...cards.merchants.slice(0,players*2),...shuffle(cards.merchants.slice(players*2))],points:shuffle(cards.points)};
  }
  async start(input:StartGameInput):Promise<GameStartResult> {
    ...
    try {return await d.roomMutationExecutor.run(input.roomId,async ():Promise<GameStartResult>=>{
      ...
      if(room?.gameType!=='CENTURY'||room.departedPlayerIds?.includes(input.actorPlayerId)||!room.players.some(p=>p.playerId===input.actorPlayerId))return failure('INVALID_PHASE');
      ...
      if(room.players.length<2||room.players.length>5)return failure('NOT_ENOUGH_PLAYERS');
      ...
      const state=createCenturyGame({...this.setup(room.players.length),gameId,playerIds:room.players.map(p=>p.playerId),now,transitionId:turnId,starter:d.random.nextInt(room.players.length)});
  ...
  async command(input:Readonly<{roomId:RoomId;actorPlayerId:PlayerId;command:CenturyClientCommand;receivedAt:ServerTime;authorization:{isCurrent():boolean}}>) {
    const parsed=v.safeParse(CenturyClientCommandSchema,input.command);if(!parsed.success)return failure('INVALID_PAYLOAD');
    ...
    try {
      const result=await d.roomMutationExecutor.run(input.roomId,async()=>{
        ...
        const applied=applyCenturyAction(s,input.actorPlayerId,c.payload,now,d.ids.generateTurnId());
        if(!applied.ok)return failure(applied.reason==='INVALID_ACTION'?'RULE_VIOLATION':applied.reason);
        const committed=await d.roomUnitOfWork.commit({roomMutation:{kind:'REPLACE',candidate:transitionCentury(room,applied.state,now),expectedRoomRevision:room.roomRevision,expectedStorageRevision:room.storageRevision},sessionMutation:{kind:'NONE'},idempotency:{scopeKey,requestId:c.requestId,payloadFingerprint,terminalResult:{outcome:'ACCEPTED'},createdAt:now}},{isSatisfied:()=>input.authorization.isCurrent()});

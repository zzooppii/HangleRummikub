...
import type { LiarDependencies } from "./service.js";

/** Process-local presence history, like the connection registry. Never owns secret words, roles or ballots. */
export class LiarHostSuccession {
  readonly offline = new Map<RoomId, Map<PlayerId, ServerTime>>();
  private timer: ReturnType<typeof setInterval> | undefined;
  constructor(private readonly deps: LiarDependencies, private readonly notify: (roomId: RoomId) => Promise<void>) {}
  ...
  resumed(roomId: RoomId, playerId: PlayerId): void {
    ...
    void this.evaluate(roomId).catch(() => console.error("LIAR Host policy reconciliation failed."));
  }
  start(): void {
    if (this.timer) return;
    this.timer = setInterval(() => {
      for (const roomId of this.offline.keys()) void this.evaluate(roomId).catch(() => console.error("LIAR Host policy retry failed."));
    }, 1000);
    this.timer.unref();
  }
  ...
  async evaluate(roomId: RoomId): Promise<boolean> {
    ...
    const transferred = await d.roomMutationExecutor.run(roomId, async () => {
      ...
      const result = await d.roomUnitOfWork.commit({ roomMutation: { kind: "REPLACE",
        ...
        idempotency: { scopeKey: `liar-host:${roomId}`, requestId: parse(RequestIdSchema, `host:${room.roomRevision}:${since}`),

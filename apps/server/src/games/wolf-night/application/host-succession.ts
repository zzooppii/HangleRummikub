...
import type { WolfDependencies } from "./service.js";

/** Process-local presence history, like the connection registry. Never owns role cards or night actions. */
export class WolfHostSuccession {
  readonly offline = new Map<RoomId, Map<PlayerId, ServerTime>>();
  private timer: ReturnType<typeof setInterval> | undefined;
  constructor(private readonly deps: WolfDependencies, private readonly notify: (roomId: RoomId) => Promise<void>) {}
  ...
  resumed(roomId: RoomId, playerId: PlayerId): void {
    ...
    void this.evaluate(roomId).catch(() => console.error("WOLF Host policy reconciliation failed."));
  }
  start(): void {
    if (this.timer) return;
    this.timer = setInterval(() => {
      for (const roomId of this.offline.keys()) void this.evaluate(roomId).catch(() => console.error("WOLF Host policy retry failed."));
    }, 1000);
    this.timer.unref();
  }
  ...
  async evaluate(roomId: RoomId): Promise<boolean> {
    ...
    const transferred = await d.roomMutationExecutor.run(roomId, async () => {
      ...
      if (!room || room.gameType !== "WOLF_NIGHT") { this.offline.delete(roomId); return false; }
      ...
      const result = await d.roomUnitOfWork.commit({ roomMutation: { kind: "REPLACE",
        ...
        idempotency: { scopeKey: `wolf-host:${roomId}`, requestId: parse(RequestIdSchema, `host:${room.roomRevision}:${since}`),

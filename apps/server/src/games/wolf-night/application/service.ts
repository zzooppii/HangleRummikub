import { GameRevisionSchema, RoomRevisionSchema, ServerTimeSchema, TurnIdSchema, RequestIdSchema,
  WolfClientCommandSchema, WolfDeckSchema, defaultWolfDeck, type WolfRole, type WolfClientCommand, type ErrorDto, type RoomId, type PlayerId, type ServerTime } from "@hangul-rummikub/shared";
...
import type { WolfRoomRecord } from "../../../model/persistence.js";
import { createWolfGame, actWolf, advanceWolf, voteWolf, sayWolf, type WolfState } from "../domain/game.js";

export type WolfDependencies = Readonly<{ roomRepository: RoomRepository; roomUnitOfWork: RoomUnitOfWork; idempotencyRepository: IdempotencyRepository;
...
export function transitionWolf(room: WolfRoomRecord, state: WolfState, at: ServerTime): Omit<WolfRoomRecord, "storageRevision"> {
  if (!room.game) throw new Error("Missing WOLF game.");
...
export class WolfService {
  ...
  constructor(readonly deps: WolfDependencies) {}
  ...
  async schedule(roomId: RoomId) {
    ...
    if (room?.gameType !== "WOLF_NIGHT" || !room.game || room.phase !== "PLAYING" || room.game.state.nextTransitionAt === null) return;
    ...
    } catch { console.error("WOLF scheduling failed; overdue recovery will retry."); }
  }
  private async cancelTimer(turnId: string) {
    try { await this.deps.turnScheduler.cancelTimeout(v.parse(TurnIdSchema, turnId)); }
    catch { console.error("WOLF timer cancellation failed; stale callbacks are guarded."); }
  }
  async start(input: StartGameInput): Promise<GameStartResult> {
    ...
    try {
      const result = await d.roomMutationExecutor.run(input.roomId, async (): Promise<GameStartResult> => {
        ...
        if (room?.gameType !== "WOLF_NIGHT") return failure("INVALID_PHASE");
        ...
        const settings = room.settings ?? { roles: null, discussionSeconds: 180 };
        const deck: WolfRole[] = [...(settings.roles ?? defaultWolfDeck(room.players.length))];
        if (deck.length !== room.players.length + 3 || !v.safeParse(WolfDeckSchema, deck).success) return failure("INVALID_PAYLOAD");
        for (let i = deck.length - 1; i > 0; i--) { const j = d.random.nextInt(i + 1); [deck[i], deck[j]] = [deck[j]!, deck[i]!]; }
        const state = createWolfGame({ gameId, playerIds: [...room.players].sort((a, b) => a.joinOrder - b.joinOrder).map(p => p.playerId),
          settings, deck, now, transitionId: turnId });
  ...
  async command(input: Readonly<{ roomId: RoomId; actorPlayerId: PlayerId; command: WolfClientCommand; receivedAt: ServerTime; authorization: { isCurrent(): boolean } }>) {
    const parsed = v.safeParse(WolfClientCommandSchema, input.command);
    ...
    try {
      const result = await d.roomMutationExecutor.run(input.roomId, async () => {
        if (!input.authorization.isCurrent()) return failure("UNAUTHENTICATED");
        const room = await d.roomRepository.findById(input.roomId);
        if (room?.gameType !== "WOLF_NIGHT" || room.departedPlayerIds?.includes(input.actorPlayerId) || !room.players.some(p => p.playerId === input.actorPlayerId)) return failure("INVALID_PHASE");
        ...
        const now = d.clock.now(); let candidate: Omit<WolfRoomRecord, "storageRevision"> = room;
        let outcome: v.InferOutput<typeof Receipt>["outcome"];
        if (c.kind === "wolf:configure") {
          ...
          if (c.kind === "wolf:rematch") {
            ...
            if (room.phase !== "PLAYING" || room.game.state.transitionId !== c.phaseId || now >= room.game.state.nextTransitionAt!) return failure("STALE_GAME_REVISION");
            let state: WolfState | null;
            if (c.kind === "wolf:act") {
              const applied = actWolf(room.game.state, input.actorPlayerId, c.phaseId, c.expectedActionRevision, c.payload, now);
              if (!applied.ok) return failure(applied.reason === "STALE" ? "STALE_GAME_REVISION" : "INVALID_PAYLOAD");
              state = applied.state;
            } else if (c.kind === "wolf:vote") state = voteWolf(room.game.state, input.actorPlayerId, c.payload.playerId, c.phaseId, now);
            else state = sayWolf(room.game.state, input.actorPlayerId, c.payload.text, c.phaseId, now);
            if (!state) return failure("INVALID_PAYLOAD");
            outcome = "ACCEPTED"; candidate = transitionWolf(room, state, now);
          }
        }
        ...
        if (room.game && (candidate.phase === "FINISHED" || candidate.game === null)) await this.cancelTimer(room.game.state.transitionId);
  ...
  async timeout(input: ScheduledTurnDeadline): Promise<{ status: "NO_OP" | "APPLIED" | "FAILED" }> {
    ...
    try {
      const applied = await d.roomMutationExecutor.run(input.roomId, async () => {
        ...
        if (room?.gameType !== "WOLF_NIGHT" || room.phase !== "PLAYING" || !room.game || room.game.gameId !== input.gameId ||
        ...
        const state = advanceWolf(previous, now, d.ids.generateTurnId(), d.random.nextInt(previous.players.length - 1), d.random.nextInt(3));
        const committed = await d.roomUnitOfWork.commit({ roomMutation: { kind: "REPLACE", candidate: transitionWolf(room, state, now),
          expectedRoomRevision: room.roomRevision, expectedStorageRevision: room.storageRevision }, sessionMutation: { kind: "NONE" },
          idempotency: { scopeKey: `wolf-timer:${room.roomId}:${room.game.gameId}`, requestId: v.parse(RequestIdSchema, `phase:${input.turnId}`),

import { createCaughtScreamSamples } from "./caught-scream.js";
...
export class LunchAudio {
  ...
  private screamSource: AudioBufferSourceNode | null = null;
  private resuming: Promise<void> | null = null;
  private audioGeneration = 0;
  get enabled() { return this.soundEnabled; }
  set enabled(value: boolean) { this.soundEnabled=value; if(!value){this.audioGeneration++;this.stopScream();} }
  private stopScream() {
    if(this.screamSource){this.screamSource.stop();this.screamSource.disconnect();this.screamSource=null;}
  }
  private playScream(ctx: AudioContext) {
    this.stopScream();
    if(!this.screamBuffer){
      const samples=createCaughtScreamSamples(ctx.sampleRate);
      this.screamBuffer=ctx.createBuffer(1,samples.length,ctx.sampleRate);
      this.screamBuffer.getChannelData(0).set(samples);
    }
    const source=ctx.createBufferSource();source.buffer=this.screamBuffer;
    source.connect(ctx.destination);this.screamSource=source;
    source.onended=()=>{source.disconnect();if(this.screamSource===source)this.screamSource=null;};
    source.start();
  }
  unlock() {
    if (!this.enabled || typeof window === "undefined" || !window.AudioContext) return;
    try { this.context ??= new AudioContext(); this.resuming=this.context.resume().catch(() => undefined); } catch { /* Audio is optional; visual feedback remains. */ }
  }
  play(cue: LunchCue) {
    const ctx = this.context;
    if (!this.enabled || !ctx) return;
    if(ctx.state!=="running") {
      // ACK may beat the gesture-triggered resume. Never replay a much later/reconnected scare.
      if(cue==="CAUGHT"&&this.resuming){
        const generation=this.audioGeneration,until=Date.now()+300;
        void this.resuming.then(()=>{if(this.enabled&&this.context===ctx&&ctx.state==="running"&&generation===this.audioGeneration&&Date.now()<=until)this.playScream(ctx);});
      }
      return;
    }
    if(cue==="CAUGHT"){this.playScream(ctx);return;}
    ...
    if(cue==="BITE"||cue==="SUSPICIOUS"||cue==="WATCHING") {
    ...
    for (const [i, hz] of LUNCH_CUES[cue].entries()) {
      ...
      osc.type = cue === "WATCHING" ? "sine" : "triangle";
  ...
  close() { this.audioGeneration++;this.stopScream();void this.context?.close().catch(() => undefined); this.context = null;this.screamBuffer=null;this.resuming=null; }
}

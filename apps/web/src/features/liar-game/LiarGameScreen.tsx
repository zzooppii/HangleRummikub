...
export const LIAR_RESULT_COPY: Readonly<Record<LiarResult["reason"], string>> = { MISIDENTIFIED: "시민이 지목되어 라이어가 살아남았습니다.", NO_VOTES: "유효한 표가 없어 라이어가 살아남았습니다.", TIE: "재투표도 동률! 라이어가 살아남았습니다.", GUESS_CORRECT: "라이어가 제시어를 맞혔습니다.", GUESS_WRONG: "라이어의 마지막 추측이 틀렸습니다.", GUESS_TIMEOUT: "라이어가 시간 안에 제시어를 맞히지 못했습니다.", CANCELLED: "참가자가 나가 이번 경기가 취소됐습니다." };
...
export function LiarGameScreen(props: LiarScreenProps) {
  ...
  const playing = game?.phase === "PLAYING" && game.stage !== "ROUND_RESULT" ? game : null;
  const roundResult = game?.phase === "PLAYING" && game.stage === "ROUND_RESULT" ? game : null;
  ...
  const scope = `${game?.gameId ?? "lobby"}:${(playing ?? roundResult)?.phaseId ?? "end"}`;
  ...
  useEffect(() => { if (game?.phase === "PLAYING") window.scrollTo({ top: 0, left: 0 }); }, [game?.gameId, game?.roundNumber]);
  ...
  return <main className="liar-shell">
    ...
    {!game ? <div className="liar-lobby"><section className="liar-invitation"><span className="liar-eyebrow">4–8명 · 10라운드 점수전</span><h2>다 아는 이야기.<br/>한 사람만 빼고.</h2><p>너무 정확하면 정답이 들키고,<br/>너무 모호하면 내가 의심받아요.</p><LiarEmblem/><div className="liar-chapters"><span>01 설명하기</span><span>02 의심하기</span><span>03 찾아내기</span></div></section>
      <section className="liar-panel"><div className="liar-section-head"><h2>오늘의 참가자</h2><span>{s.room.players.length} / 8명</span></div><div className="liar-roster">{s.room.players.map((p, i) => <div key={p.playerId}><span className="liar-avatar">{i + 1}</span><span><b>{p.nickname}{p.playerId === self ? " · 나" : ""}</b><small>{p.isHost ? "방장 · " : ""}{p.connectionStatus === "CONNECTED" ? "접속 중" : "연결 기다리는 중"}</small></span></div>)}</div>
        <fieldset className="liar-settings" disabled={!host || !enabled}><legend>10라운드의 주제</legend><div className="liar-options">{(["RANDOM", ...LIAR_CATEGORIES] as const).map(category => <button key={category} aria-pressed={settings.category === category} onClick={() => configure({ ...settings, category })}>{LIAR_CATEGORY_LABELS[category]}</button>)}</div></fieldset>
        ...
        <p className="liar-hint">시민 승리 각 +1점 · 라이어 승리 +3점<br/>제시어 중복 없이 10라운드를 진행합니다.</p><p className="liar-hint">{ready.guidance}</p>{host ? <button className="liar-primary liar-wide" disabled={!ready.canStart} onClick={props.onStart}>10라운드 시작 · 비밀 카드 나누기 →</button> : <p>방장이 게임을 시작하면 카드를 받습니다.</p>}
      </section></div> : <>
      <section className={`liar-stage${playing ? " liar-live" : ""}`}><div><span className="liar-eyebrow">{game.roundNumber} / {game.totalRounds} 라운드 · {LIAR_CATEGORY_LABELS[game.category]}</span><h2>{playing ? LIAR_STAGE_COPY[playing.stage] : game.phase === "FINISHED" && game.result.reason === "CANCELLED" ? "이번 경기는 여기까지" : game.phase === "FINISHED" ? "10라운드 최종 결과" : "이번 라운드의 진실"}</h2></div>{playing && <div className={`liar-timer${seconds <= 10 ? " urgent" : ""}`} role="timer" aria-label={`남은 시간 ${seconds}초`}>{Math.floor(seconds / 60)}:{String(seconds % 60).padStart(2, "0")}<small>{seconds ? "남은 시간" : "결과 확인 중"}</small></div>}</section>
      {"result" in game ? <section className="liar-panel liar-result"><span className="liar-answer">{game.result.word}</span><h2>{game.result.reason !== "CANCELLED" && `${game.roundNumber}라운드 · `}{game.result.reason === "CANCELLED" ? "승패 없이 종료" : game.result.winnerPlayerIds.includes(game.result.liarPlayerId) ? "라이어 승리!" : "시민 승리!"}</h2><p>{LIAR_RESULT_COPY[game.result.reason]}</p><p>라이어는 <b>{name(game.result.liarPlayerId)}</b>님이었어요.</p>{game.result.guess && <p>마지막 추측: <b>{game.result.guess}</b></p>}
        ...
        <LiarScoreboard game={game} name={name}/>
        {game.phase === "FINISHED" ? <>{game.result.reason !== "CANCELLED" && <p className="liar-champion">{game.matchWinnerPlayerIds.length > 1 ? "공동 우승" : "최종 우승"} · {game.matchWinnerPlayerIds.map(name).join(", ")}</p>}{host ? <button className="liar-primary" disabled={!enabled} onClick={props.onRematch}>같은 방에서 다시 하기</button> : <p>방장이 다음 경기를 준비할 때까지 기다려주세요.</p>}</> : host ? <button className="liar-primary" disabled={!enabled} onClick={() => { void command({ ...envelope(), kind: "liar:nextRound", gameId: game.gameId, phaseId: game.phaseId, payload: {} }); }}>{game.roundNumber + 1}라운드 시작 →</button> : <p>방장이 다음 라운드를 시작할 때까지 기다려주세요.</p>}
        <details><summary>완료한 라운드 기록 · {game.rounds.length}회</summary><ol className="liar-round-history">{game.rounds.map(round => <li key={round.roundNumber}><b>{round.roundNumber}라운드 · {round.result.word}</b><span>라이어: {name(round.result.liarPlayerId)} · {round.result.winnerPlayerIds.includes(round.result.liarPlayerId) ? "라이어 승리" : "시민 승리"}</span></li>)}</ol></details>
        ...
        <LiarScoreboard game={game} name={name}/><section className="liar-panel liar-chat"><h3>자유 토론</h3><div className="liar-messages" role="log" aria-label="토론 채팅">{game.messages.length === 0 ? <p className="liar-hint">{game.stage === "DISCUSSION" ? "첫 이야기를 들려주세요." : "설명이 끝나면 토론할 수 있어요."}</p> : game.messages.map((m, i) => <p key={`${m.at}-${i}`}><b>{name(m.playerId)}</b><span>{m.text}</span></p>)}</div><form onSubmit={e => { e.preventDefault(); if (!message.trim() || !canAct || game.stage !== "DISCUSSION") return; const submitted = message; void command({ ...envelope(), kind: "liar:say", gameId: game.gameId, phaseId: game.phaseId, payload: { text: submitted } }).then(ok => { if (ok) setMessage(current => current === submitted ? "" : current); }); }}><label htmlFor="liar-message">토론 메시지 · 200자 이내</label><input id="liar-message" maxLength={200} value={message} disabled={!canAct || game.stage !== "DISCUSSION"} onChange={e => setMessage(e.target.value)} placeholder={game.stage === "DISCUSSION" ? "누가 의심되나요?" : "토론 시간에 열립니다"}/><button disabled={!canAct || game.stage !== "DISCUSSION" || !message.trim()}>보내기</button></form></section>
      </aside></div>}
    </>}
    <details className="liar-help"><summary>게임 방법과 온라인 진행 안내</summary><div><p>4~8명 중 한 명만 제시어를 모릅니다. 시민은 라이어를 찾고, 라이어는 정체를 숨기거나 마지막에 정답을 맞히면 승리합니다.</p><ol><li>15초 동안 개인 카드를 확인합니다. 라이어는 주제만 알 수 있어요.</li><li>순서대로 30초 안에 40자 이내로 설명합니다. 제시어를 직접 말하지 마세요.</li><li>설명 후 정해진 시간 동안 자유롭게 토론합니다. 채팅은 1초에 한 번 보낼 수 있어요.</li><li>30초 안에 자신을 제외한 한 명에게 비밀 투표합니다. 확정 후 변경할 수 없습니다.</li><li>동률은 해당 후보에게 한 번 재투표합니다. 재동률이나 전원 기권이면 라이어 승리입니다.</li><li>시민이 지목되면 라이어 승리. 라이어가 잡히면 20초 안에 정답을 한 번 추측합니다.</li></ol><p>미설명은 건너뛰고 미투표는 기권입니다. 정답의 공백·대소문자와 등록된 동의어는 허용하지만 비슷한 단어를 자동으로 정답 처리하지 않습니다. 연결이 잠깐 끊겨도 같은 자리로 돌아올 수 있으며 시간은 계속 흐릅니다. 한 경기는 10라운드입니다. 시민 승리는 시민 각 +1점, 라이어 승리는 라이어 +3점이며 동점은 공동 우승입니다. 결과를 확인한 뒤 방장이 다음 라운드를 시작합니다. 나가기를 누르면 경기가 취소되고 완료한 라운드 점수만 참고용으로 남습니다. 내장 음성은 제공하지 않습니다.</p></div></details>
    <footer className="liar-footer">하나의 제시어, 여럿의 이야기, 단 한 명의 라이어.</footer>
  </main>;
}

function LiarScoreboard({ game, name }: { game: NonNullable<LiarWebSnapshot["game"]>; name(id: string): string }) {
  const scores = [...game.scores].sort((a, b) => b.points - a.points);
  const result = "result" in game ? game.result : null;
  return <section className="liar-panel liar-scoreboard" aria-label="누적 점수"><h3>{game.phase === "FINISHED" ? "최종 순위" : "누적 순위"}</h3><ol>{scores.map(score => {
    const rank = 1 + scores.filter(other => other.points > score.points).length;
    const gain = result?.winnerPlayerIds.includes(score.playerId) ? score.playerId === result.liarPlayerId ? 3 : 1 : 0;
    return <li key={score.playerId}><span className="liar-rank">{rank}위</span><b>{name(score.playerId)}</b>{result && <small>이번 +{gain}</small>}<strong>{score.points}점</strong></li>;
  })}</ol>{result?.reason === "CANCELLED" && <p className="liar-hint">경기 취소 · 완료한 라운드의 참고 점수입니다.</p>}</section>;
}

...
export function useVegasClock(serverTime: number, deadlineAt: number | null) {
    ...
    return deadlineAt === null ? 0 : Math.max(0, Math.min(30, Math.ceil((deadlineAt - Math.max(now, serverTime)) / 1000)));
}

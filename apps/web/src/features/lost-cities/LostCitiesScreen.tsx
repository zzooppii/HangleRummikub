...
import { useLostCitiesCountdown } from "./turn-timer.js";
import { formatCountdownMmSs } from "../../lib/turn-countdown.js";
import { useLostCitiesSound, type LostCitiesSoundCue } from "./sound.js";
...
export function LostCitiesScreen(props:Props) {
  ...
  const sound=useLostCitiesSound(s.game,s.self.playerId,props.connected);
...
function LostCitiesTable({game:g,playSound,...props}:Props&{game:LostCitiesProjection;playSound(cue:LostCitiesSoundCue):void}) {
  ...
  const isMyTurn=g.phase==='PLAYING'&&g.activePlayerId===selfId;
  const remaining=useLostCitiesCountdown(g.phase==='PLAYING'?g.deadlineAt:null,props.snapshot.serverTime);
  const warningScope=g.phase==='PLAYING'?g.turnId:null;
  const warning=useRef<{turn:string|null;level:number}>({turn:null,level:0});
  useEffect(()=>{
    if(warning.current.turn!==warningScope)warning.current={turn:warningScope,level:0};
    const level=remaining>0&&remaining<=5?2:remaining>0&&remaining<=10?1:0;
    if(level>warning.current.level){warning.current.level=level;if(isMyTurn&&props.connected)playSound('WARNING');}
  },[remaining,warningScope,isMyTurn,props.connected,playSound]);
  const canAct=isMyTurn&&remaining>0&&props.connected&&!props.pending&&!flight&&!retry;
  ...
  async function send(c:LostCitiesClientCommand) {
    ...
    if(c.kind==='lostCities:nextRound')playSound('SELECT');
    ...
    catch(error){if(mounted.current&&scopeRef.current===sentScope){playSound('ERROR');if(error instanceof LostCitiesCommandRejected){setRetry(null);setMessage(error.message);}else{setRetry(c);setMessage('응답을 확인하지 못했습니다. 같은 요청의 결과를 다시 확인해주세요.');}}}
  ...
  return <>
    {g.phase==='PLAYING'&&<div className={`lc-turn-banner ${isMyTurn?'lc-turn-mine':'lc-turn-other'} ${remaining<=10?'lc-turn-urgent':''}`}>
      <span className="lc-turn-icon" aria-hidden="true">{isMyTurn?'➤':'◷'}</span>
      <div className="lc-turn-copy" role="status"><strong>{status}</strong><span>{!props.connected?'연결 복구 중 · 지금은 행동할 수 없습니다':remaining===0?'시간 종료 · 서버의 자동 행동을 기다립니다':flight?'턴 제출 중 · 서버 확인을 기다려주세요':retry?'제출한 턴의 결과를 다시 확인해주세요':isMyTurn?'카드 1장 → 놓기 / 버리기 → 가져오기 → 턴 확정':'상대가 카드를 고르고 있습니다'}</span></div>
      <div className="lc-turn-clock"><small>남은 시간 · 60초</small><strong role="timer" aria-label={`남은 시간 ${remaining}초`}>{formatCountdownMmSs(remaining)}</strong><small>시간 초과 시 자동 버리기 + 덱 1장</small></div><progress className="lc-turn-progress" aria-label="남은 턴 시간" max={60} value={remaining}/>
    </div>}
    ...
    <div className="lc-board-scroll" tabIndex={0} aria-label="탐험 보드 가로 스크롤"><div className={`lc-board ${g.settings?.mode==='SIX_EXPEDITIONS'?'lc-board-six':''}`} style={{gridTemplateColumns:`repeat(${lostCitiesSuits(g.settings?.mode).length},minmax(0,1fr))`}} aria-label={g.settings?.mode==='SIX_EXPEDITIONS'?'여섯 탐험 보드':'다섯 탐험 보드'}>{lostCitiesSuits(g.settings?.mode).map(suit=>{
      ...
      return <section className={`lc-lane lc-suit-${suit}`} key={suit} aria-label={LC_LABELS[suit]}><header><span aria-hidden="true">{LC_MARKS[suit]}</span><strong>{LC_LABELS[suit]}</strong></header><Expedition expedition={other.expeditions.find(e=>e.suit===suit)!} highlight={g.feedback?.playerId===other.playerId&&g.feedback.kind==='PLAY'&&g.feedback.card.suit===suit}/><div className="lc-discard-zone"><span className="lc-zone-label">공용 버림</span><button type="button" className={`lc-discard-slot ${discard.top?'lc-has-card':''}`} disabled={disabled} aria-pressed={draft.draw?.kind==='DISCARD'&&draft.draw.suit===suit} aria-label={`${LC_LABELS[suit]} 버린 카드 ${discard.top?cardLabel(discard.top):'없음'} 가져오기`} onClick={()=>{playSound('SELECT');setDraft(d=>({...d,draw:{kind:'DISCARD',suit}}));}}>{discard.top?<CardFace card={discard.top}/>:<span className="lc-empty-mark" aria-hidden="true">{LC_MARKS[suit]}</span>}</button><span className="lc-discard-count">{discard.count}장</span></div><Expedition expedition={me.expeditions.find(e=>e.suit===suit)!} highlight={!!card&&draft.kind==='PLAY'&&card.suit===suit&&playable}/></section>;
    })}</div></div>
    {g.settings?.mode==='SIX_EXPEDITIONS'&&<p className="lc-scroll-hint">보드를 좌우로 밀어 여섯 탐험을 확인하세요 ↔</p>}<div className="lc-table-status"><span className={isMyTurn?'lc-your-turn':''} role="status">{status}</span><span className="lc-board-legend">위: 상대 탐험 · 아래: 내 탐험</span><button className={`lc-deck ${g.deckCount<=8?'lc-deck-low':''}`} type="button" disabled={!canAct||!draft.kind} aria-pressed={draft.draw?.kind==='DECK'} onClick={()=>{playSound('SELECT');setDraft(d=>({...d,draw:{kind:'DECK'}}));}}><span className="lc-deck-icon" aria-hidden="true">✦</span><span>덱에서 가져오기<strong>{g.deckCount}장 남음</strong></span></button></div>
    {feedback&&<p className="lc-feedback" key={`${g.gameRevision}-feedback`} role="status">{feedback}</p>}
    <section className="lc-hand-section" aria-label="내 손패"><div className="lc-hand-heading"><h2>내 손패 <small>8장</small></h2><span>나에게만 보입니다</span></div><div className="lc-hand">{sortLostCitiesHand(g.privateState.hand).map(c=><button type="button" key={c.cardId} className={`lc-hand-card lc-suit-${c.suit}`} aria-label={cardLabel(c)} aria-pressed={draft.cardId===c.cardId} disabled={!canAct} onClick={()=>{playSound('SELECT');setDraft({cardId:c.cardId,kind:null,draw:null});setMessage(null);}}><CardFace card={c}/></button>)}</div></section>
    {g.phase==='PLAYING'&&<footer className={`lc-action-bar ${isMyTurn?'lc-action-mine':''}`}><strong className="lc-action-turn"><span>{status}</span><span className={remaining<=10?'lc-time-urgent':''}>남은 시간 {formatCountdownMmSs(remaining)}</span></strong><div className="lc-action-summary"><span className="lc-eyebrow">{!card?'01 · 카드 선택':!draft.kind?'02 · 행동 선택':!draft.draw?'03 · 가져올 곳':'04 · 턴 확정'}</span><p aria-live="polite">{canAct?preview.hint:remaining===0?'시간 종료 · 서버의 자동 행동을 기다립니다.':flight?'서버에서 결과를 확인하는 중…':retry?'요청 결과를 재확인해주세요.':props.connected?'상대의 탐험을 살펴보세요.':'연결을 복구하는 중입니다.'}</p></div><div className="lc-action-buttons"><button type="button" disabled={!canAct||!card||!playable} aria-pressed={draft.kind==='PLAY'} onClick={()=>{playSound('SELECT');setDraft(d=>({...d,kind:'PLAY',draw:null}));}}>탐험에 놓기</button><button type="button" disabled={!canAct||!card} aria-pressed={draft.kind==='DISCARD'} onClick={()=>{playSound('SELECT');setDraft(d=>({...d,kind:'DISCARD',draw:null}));}}>버리기</button><button type="button" className="lc-primary" disabled={!canAct||!preview.action} onClick={submit}>턴 확정 <span aria-hidden="true">→</span></button></div>{card&&!playable&&<small className="lc-placement-hint">{card.kind==='INVESTMENT'?'숫자를 놓은 탐험에는 투자할 수 없습니다.':'이미 놓인 숫자보다 큰 카드만 놓을 수 있습니다.'} 버리기는 가능합니다.</small>}</footer>}

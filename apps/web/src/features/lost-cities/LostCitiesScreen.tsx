...
import { LOST_CITIES_SUITS, PROTOCOL_VERSION, type LostCitiesCard, type LostCitiesClientCommand, type LostCitiesProjection, type LostCitiesSuit, type PlatformPlayerViewV2 } from "@hangul-rummikub/shared";
...
const artStyle=(suit:LostCitiesSuit):CSSProperties=>({backgroundPosition:`${LOST_CITIES_SUITS.indexOf(suit)*25}% 42%`});
...
function LostCitiesTable({game:g,...props}:Props&{game:LostCitiesProjection}) {
  ...
  return <>
    <div className="lc-player-bar"><div className="lc-player"><span className="lc-avatar">{name(other.playerId).slice(0,1)}</span><div><strong>{name(other.playerId)}</strong><small>{players.find(p=>p.playerId===other.playerId)?.connectionStatus==='CONNECTED'?'상대 탐험 · 손패 8장':'재접속을 기다리는 중'}</small></div><div className="lc-hidden-hand" aria-hidden="true">{Array.from({length:8},(_,i)=><span key={i}/>)}</div></div><div className="lc-round"><span>ROUND</span><strong>{g.round} <small>/ 3</small></strong></div><div className="lc-match-score"><span>누적 점수</span><strong>나 {me.cumulative} <i>:</i> 상대 {other.cumulative}</strong></div></div>
    ...
    <div className="lc-board" aria-label="다섯 탐험 보드">{LOST_CITIES_SUITS.map(suit=>{

  ...
  return <main className="app-shell city-shell city-finished-shell">
    ...
    <ol className="city-results" aria-label="최종 순위와 점수">{game.result.rankings.map(entry => {
      ...
      return <li className={`city-result-row${entry.winner ? " is-winner" : ""}`} key={entry.playerId}><header><div><span className="city-result-rank">{entry.rank}위{entry.winner ? " · 우승" : ""}</span><h3>{participant?.nickname ?? "참가자"}{entry.playerId === self.playerId ? " · 나" : ""}</h3><p>{entry.forfeited ? "기권 · " : ""}건물 {entry.buildingCount}개</p></div><strong className="city-result-score">{entry.score}<small>점</small></strong></header>
        <dl className="city-score-details"><div><dt>건물 점수</dt><dd>{entry.buildingVP}</dd></div><div><dt>완성 보너스</dt><dd>+{entry.completionBonus}</dd></div><div><dt>다양성 보너스</dt><dd>+{entry.diversityBonus}</dd></div></dl>
        {player !== undefined && player.builtBuildings.length > 0 ? <div className="city-card-grid city-built-grid">{player.builtBuildings.map(card => <div className="city-building is-built" key={card.cardId} aria-label={cityCardLabel(card)}><CityBuildingFace card={card} /></div>)}</div> : null}
      </li>;
    })}</ol>
    <details className="city-panel city-frozen-private"><summary>내 마지막 비공개 정보 보기</summary><p>다른 참가자의 손패와 비밀 역할은 게임이 끝나도 공개하지 않습니다.</p><p>내 역할: {game.privateState.selectedRoleIds.length === 0 ? "없음" : game.privateState.selectedRoleIds.map(cityRoleLabel).join(" · ")}</p>
      <h3>남은 내 손패 {game.privateState.hand.length}장</h3><div className="city-card-grid">{game.privateState.hand.map(card => <div className="city-building" key={card.cardId} aria-label={cityCardLabel(card)}><CityBuildingFace card={card} /></div>)}</div>
      {game.privateState.pendingCards !== undefined ? <><h3>종료 시 선택 대기 중이던 카드</h3><p>종료된 게임에서는 더 이상 선택하지 않습니다.</p><div className="city-card-grid">{game.privateState.pendingCards.map(card => <div className="city-building" key={card.cardId} aria-label={cityCardLabel(card)}><CityBuildingFace card={card} /></div>)}</div></> : null}
    </details>

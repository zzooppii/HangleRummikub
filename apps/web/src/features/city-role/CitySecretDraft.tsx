...
export function CitySecretDraft({ roles, locked, onAction }: Readonly<{
  ...
  return <div className="city-secret-draft">
    ...
    <div className="city-draft-progress" role="status"><span>01 · {keep ? `${CITY_ROLE_HELP[keep].name} 가져오기` : "가져갈 역할 선택"}</span><span>02 · {discard ? `${CITY_ROLE_HELP[discard].name} 버리기` : "버릴 역할 선택"}</span></div>
    <div className="city-role-grid">{roles.map(role => <article key={role} data-role={role} className={`city-role-card${keep === role ? " is-kept" : ""}${discard === role ? " is-discarded" : ""}`}>
      <CityRoleEmblem roleId={role} /><span className="city-role-order">등장 순서 {Number(role.slice(-2))}</span><strong>{CITY_ROLE_HELP[role].name}</strong><p>{CITY_ROLE_HELP[role].summary}</p>
      <div className="city-draft-card-actions"><button type="button" aria-label={`${cityRoleLabel(role)} 가져오기`} aria-pressed={keep === role} disabled={locked} onClick={() => { setKeep(role); if (discard === role) setDiscard(null); }}>가져오기</button>
        <button type="button" aria-label={`${cityRoleLabel(role)} 비공개 버리기`} aria-pressed={discard === role} disabled={locked} onClick={() => { setDiscard(role); if (keep === role) setKeep(null); }}>비공개 버리기</button></div>
    </article>)}</div>

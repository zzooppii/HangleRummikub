...
import { CITY_DEFAULT_SETTINGS, CITY_DEFAULT_SELECTION_SECONDS, CITY_SELECTION_TIME_OPTIONS, CITY_EXPANDED_ROLES, CITY_STANDARD_CAST, type CityExpansionSettings, type CityExpansionClientCommand, type RoomRevision } from '@hangul-rummikub/shared';
...
export function CityExpansionLobby({ settings, revision, host, connected, count, onCommand }: { settings: CityExpansionSettings | undefined; revision: RoomRevision; host: boolean; connected: boolean; count: number; onCommand(command: CityExpansionClientCommand): Promise<void> }) {
  ...
  async function save(next: CityExpansionSettings) {
    ...
    try { await onCommand({ kind: 'city:configure', protocolVersion: 1, requestId: createRequestId(), expectedRoomRevision: revision, payload: { enabled: next.enabled, roles: [...next.roles], selectionSeconds: next.selectionSeconds ?? CITY_DEFAULT_SELECTION_SECONDS } }); }
  ...
  return <section className="city-expansion-lobby">
    ...
    <div className="city-mode-switch" aria-label="게임 모드">
      <button aria-pressed={!current.enabled} disabled={!host || !connected || busy} onClick={() => void save({ ...current, enabled: false, roles: CITY_STANDARD_CAST })}><strong>일반판</strong><span>기본 직업 8개와 추천 특수 건물 14종</span></button>
      <button aria-pressed={current.enabled} disabled={!host || !connected || busy} onClick={() => void save({ ...current, enabled: true })}><strong>확장판</strong><span>직업을 직접 선택 · 특수 건물 30종 중 무작위 14종</span></button>
    </div>
    <div className="city-cast-settings city-selection-time"><label><span>직업 선택 시간</span><select aria-label="직업 선택 시간" value={current.selectionSeconds ?? CITY_DEFAULT_SELECTION_SECONDS} disabled={!host || !connected || busy} onChange={event => {
      const selectionSeconds = CITY_SELECTION_TIME_OPTIONS.find(seconds => seconds === Number(event.target.value));
      if (selectionSeconds !== undefined) void save({ ...current, selectionSeconds });
    }}>{CITY_SELECTION_TIME_OPTIONS.map(seconds => <option key={seconds} value={seconds}>{seconds}초{seconds === CITY_DEFAULT_SELECTION_SECONDS ? ' · 기본' : ''}</option>)}</select><small>각 직업을 고를 때 적용됩니다. 행동 시간은 90초입니다.</small></label></div>
    {current.enabled && <div className="city-cast-settings">{Array.from({ length: 9 }, (_, i) => <label key={i}><span>{String(i + 1).padStart(2, '0')}번 직업</span><select aria-label={`${i + 1}번 직업`} value={current.roles[i] ?? ''} disabled={!host || !connected || busy} onChange={event => {
      ...
      void save({ ...current, enabled: true, roles });

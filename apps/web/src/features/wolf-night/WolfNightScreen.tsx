...
export function WolfNightScreen(props: WolfScreenProps) {
  ...
  return <main className={`wolf-shell${isDay ? " wolf-day" : ""}`}>
    ...
    {!game ? <div className="wolf-lobby">
      ...
      <section className="wolf-stage-banner"><div><span className="wolf-eyebrow">{game.phase === "FINISHED" ? "THE TRUTH" : isDay ? "DAYBREAK" : "AFTER DARK"}</span><h2>{STAGE_COPY[stage]}</h2></div>{playing && <div className={`wolf-timer${seconds <= 10 ? " urgent" : ""}`} role="timer" aria-label={`남은 시간 ${seconds}초`}>{Math.floor(seconds / 60)}:{String(seconds % 60).padStart(2,"0")}<small>남은 시간</small></div>}</section>

...
export const SPYFALL_STAGE_COPY: Record<SpyfallStage, string> = { REVEAL: "당신의 임무를 확인하세요", QUESTION: "질문 속에 단서를 숨기세요", ANSWER: "자연스럽게 대답하세요", ACCUSATION: "이 사람, 스파이일까요?", FINAL_ACCUSATION: "마지막으로 지목할 시간", GUESS: "스파이의 마지막 추리" };
...
export function SpyfallGameScreen(props: SpyfallScreenProps) {
  ...
  return <main className="spy-shell" onPointerDownCapture={audio.unlock} onKeyDownCapture={audio.unlock}>
    ...
    {!game ? <div className="spy-lobby">
      ...
      <div className="spy-play-layout"><div className="spy-main">
        ...
        <section className="spy-dossier spy-action" aria-live="polite">
          ...
          {game.stage === "GUESS" && <><h3>{name(game.revealedSpyId)}님이 스파이였습니다.</h3>{game.privateView.role === "SPY" ? <><p>20초 안에 장소를 한 번 맞히면 스파이 승리입니다. 틀리거나 시간이 지나면 시민이 승리합니다. 아래 장소 카드를 선택한 뒤 확정하세요.</p><button className="spy-primary" disabled={!canAct || !location} onClick={() => identity && location && void command({ ...identity, kind: "spyfall:guess", payload: { location } })}>{location ? `${SPYFALL_LOCATION_LABELS[location]} · 최종 추측 확정` : "아래에서 장소를 선택하세요"}</button></> : <p>아직 장소를 말하지 마세요. 스파이의 최종 추측을 기다립니다.</p>}</>}
    ...
    <details className="spy-help"><summary>처음이라면 · 게임 방법과 온라인 진행 안내</summary><div><ol><li>한 명은 스파이, 나머지는 같은 장소를 아는 참가자입니다. 역할을 켜면 직업에 맞춰 대답해보세요.</li><li>질문받은 사람이 다음 질문자가 됩니다. 바로 되묻기는 금지합니다. 상대 선택·답변은 각각 최대 60초이며 전체 질문 시간이 먼저 끝나면 최종 지목으로 넘어갑니다.</li><li>도중 지목은 각자 한 번. 피지목자를 제외한 전원 동의 시 정체를 공개합니다. 지목자는 자동 찬성, 미응답은 동의하지 않은 것으로 처리합니다.</li><li>스파이는 질문 시간이 흐를 때 정체를 공개하고 장소를 한 번 추측할 수 있습니다. 투표로 잡힌 뒤에도 마지막 추측 기회가 주어집니다. 20초 안에 맞히면 스파이 승리, 틀리거나 시간이 지나면 시민 승리입니다. 추측이 끝날 때까지 장소를 말하지 마세요.</li><li>전체 시간이 끝나면 순서대로 마지막 지목 기회를 갖습니다. 지목 선택 20초, 투표 30초. 끝내 합의하지 못하면 스파이 승리입니다.</li><li>창을 닫아도 시간은 흐릅니다. 재접속하면 같은 임무로 돌아옵니다. 나가기를 누르면 이번 판은 취소됩니다.</li></ol><p>브라우저 내 음성통화는 제공하지 않습니다. 효과음은 첫 화면 조작 이후 재생되며 상단에서 음량을 조절할 수 있습니다.</p></div></details>

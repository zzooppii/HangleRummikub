...
export function HalliGalliScreen(props: Props) {
 ...
 const canFlip = canBell && playing?.activePlayerId === self && now >= playing.flipAvailableAt;
 ...
 return <main className="halli-screen">
  ...
  {!game ? <div className="halli-lobby"><section className="halli-welcome"><span className="halli-eyebrow">눈은 과일에. 손은 벨에.</span><h2>하나, 둘, 셋, 넷…<br/><em>다섯!</em></h2><div className="halli-hero-art"><FruitCard card={{ fruit: "STRAWBERRY", count: 2 }}/><HalliBellArt/><FruitCard card={{ fruit: "STRAWBERRY", count: 3 }}/></div><p>같은 과일이 정확히 5개라면?<br/>가장 먼저 벨을 울려 카드를 가져오세요.</p><span className="halli-tag">2–6명 · 반응 속도 · 최대 15분</span></section>

...
export const CityRoleIdSchema = v.picklist(CITY_ROLE_IDS);
...
export const CityBuildingTemplateIdSchema = v.pipe(v.string(), v.regex(/^CB-(?:CIV|CUL|TRA|GUA|LAN)-0[1-6]$/u));
...
export const CityPublicBuildingSchema = v.pipe(v.strictObject({
  ...
  cost: v.pipe(v.number(), v.integer(), v.minValue(1), v.maxValue(6)),
  victoryPoints: v.pipe(v.number(), v.integer(), v.minValue(1), v.maxValue(6)),
}), v.check(card => card.cost === card.victoryPoints, "CITY printed cost and VP must match."));
...
export const CityRoleAbilityPayloadSchema = v.variant("ability", [
  ...
  v.strictObject({ ability: v.literal("REPLACE_OWN_CARDS"), cardIds: v.pipe(v.array(CityBuildingCardIdSchema), v.minLength(1), v.maxLength(60), v.check(ids => new Set(ids).size === ids.length)) }),

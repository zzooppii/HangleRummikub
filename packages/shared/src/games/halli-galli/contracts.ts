...
export const HalliResultSchema = v.strictObject({ reason: v.picklist(["LAST_PLAYER", "CANCELLED"]), winnerPlayerIds: v.array(PlayerIdSchema), scores: v.array(v.strictObject({ playerId: PlayerIdSchema, cards: Count })) });
const Base = { gameType: v.literal("HALLI_GALLI"), gameId: GameIdSchema, gameRevision: GameRevisionSchema, rulesVersion: v.literal("halli-galli-v2"),
...
export const HalliPlayingProjectionSchema = v.strictObject({ ...Base, phase: v.literal("PLAYING"), turnId: TurnIdSchema, activePlayerId: PlayerIdSchema, flipAvailableAt: ServerTimeSchema, deadlineAt: ServerTimeSchema });

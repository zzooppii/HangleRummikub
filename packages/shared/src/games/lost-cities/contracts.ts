...
import { LostCitiesCardSchema, LostCitiesSuitSchema, LOST_CITIES_SUITS, type LostCitiesCard } from "./actions.js";

const count = v.pipe(v.number(), v.safeInteger(), v.minValue(0));
const points = v.pipe(v.number(), v.safeInteger(), v.minValue(-1200), v.maxValue(2400));
...
export const LostCitiesRoundResultSchema = v.strictObject({
  round, scores: v.pipe(v.array(v.strictObject({ playerId: PlayerIdSchema,
    expeditions: v.pipe(v.array(LostCitiesExpeditionScoreSchema), v.length(5)), total: points, cumulative: points })), v.length(2)),
});
...
const Base = {
  ...
  rulesVersion: v.literal("lost-cities-base-v1"), round, roundId: TurnIdSchema, deckCount: v.pipe(count, v.maxValue(44)),
  discards: v.pipe(v.array(v.strictObject({ suit: LostCitiesSuitSchema, count: v.pipe(count, v.maxValue(12)), top: v.nullable(LostCitiesCardSchema) })), v.length(5)),
  playerStates: v.pipe(v.array(v.strictObject({ playerId: PlayerIdSchema, handCount: v.literal(8), cumulative: points,
    expeditions: v.pipe(v.array(v.strictObject({ suit: LostCitiesSuitSchema, cards: v.pipe(v.array(LostCitiesCardSchema), v.maxLength(12)), score: LostCitiesExpeditionScoreSchema })), v.length(5)),
  })), v.length(2)),
...
function uniqueSuits(values: readonly {suit:string}[]):boolean { return values.length===5 && new Set(values.map(x=>x.suit)).size===5; }
...
export function lostCitiesProjectionIsConsistent(g: LostCitiesProjection): boolean {
  ...
  if(players.size!==2||!players.has(g.privateState.playerId)||!uniqueSuits(g.discards))return false;
  ...
  if(g.deckCount+16+g.discards.reduce((n,d)=>n+d.count,0)+g.playerStates.reduce((n,p)=>n+p.expeditions.reduce((m,e)=>m+e.cards.length,0),0)!==60)return false;
  if(g.discards.some(d=>(d.count===0)!==(d.top===null)||(d.top!==null&&d.top.suit!==d.suit)))return false;
  for(const p of g.playerStates) {
    if(!uniqueSuits(p.expeditions)||p.cumulative!==g.roundResults.reduce((n,r)=>n+(r.scores.find(s=>s.playerId===p.playerId)?.total??0),0))return false;

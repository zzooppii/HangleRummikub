...
export const SaboteurFeedbackSchema = v.nullable(v.strictObject({ playerId: PlayerIdSchema, kind: v.picklist(['PLACE', 'BREAK', 'REPAIR', 'MAP', 'ROCKFALL', 'DISCARD', 'TAKE_GOLD']), at: ServerTimeSchema, targetPlayerId: v.nullable(PlayerIdSchema), tool: v.nullable(SaboteurToolSchema), position: v.nullable(v.strictObject({ x: SaboteurCoordinateSchema, y: SaboteurCoordinateSchema })) }));
const base = { gameType: v.literal('SABOTEUR'), gameId: GameIdSchema, gameRevision: GameRevisionSchema, rulesVersion: v.literal('saboteur-base-2025-v1'), round: v.pipe(count, v.minValue(1), v.maxValue(3)), roundId: TurnIdSchema,
